/*
10s阅读
立即参与 -> http://h5.0w3kud.top/j/r2?upuid=240587&ch=gzh
备用链接 -> http://h5.0zp8vl.top/j/r2?upuid=240587&ch=gzh
每小时有0.3 一天5轮 一天1.5
...
const jrpush = false;
const UA = "Mozilla/5.0 (Linux; Android 11; Redmi K30 Build/RKQ1.200826.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/77.0.3865.120 MQQBrowser/6.2 TBS/045617 Mobile Safari/537.36 MMWEBID/5077 MicroMessenger/8.0.6.1900(0x2800063D) Process/tools WeChat/arm64 Weixin NetType/WIFI Language/zh_CN ABI/arm64";
let host = $.getdata("read10surl")
  ? $.getdata("read10surl")
  : `http://m.xhh25.top`;
let cookiesArr = [$.getdata("read10sck")];
if ($.isNode()) {
  cookiesArr = "udtauth=f03cwPeqibePboH0rDJUwPNZ92Khl%2BKmEmBAuvAwZg%2FEk1AmORS0%2B45qmIosbcuW0s7S1e%2BZWl2BRvsjUUR00NTtr0TM3qlOOKYRX3SKxSrNGkBa8%2BfNs21UGKzl%2BAS%2BDEZSOEGiswo2jn%2FSWQnXy%2FgaEZ%2FWzZmbN5Y8w8oGjTY; PHPSESSID=c5uma0qol1fbodoqlv5httspic".split("@");
...
!(async () => {
  ...
  if (!cookiesArr[0]) {
    $.msg(
      $.name,
      "【提示】请先获取cookie",
      "微信打开 http://h5.hakc.top/j/r1?upuid=136678&ch=xmy",
      {
...
function read10sck() {
  if ($request.url.indexOf("do_read") > -1) {
    ...
    if ($request.headers.Cookie)
      $.setdata($request.headers.Cookie, `read10sck`);
    $.log(read10sck);
...
function read(url1) {
  return new Promise(async (resolve) => {
    if (!url1) {
      url = `${host}/read_channel/do_read&pageshow&r=0.7865591251661564`;
    } else {
      url = url1;
    }
    let headers = {
...
function getNextTime() {
  return new Promise(async (resolve) => {
    url = `${host}/read_channel/finish?_t=`;
    let headers = {
      cookie,
      referer: url,
      "X-Requested-With": "XMLHttpRequest",
    ...
    $.get(options, async (err, resp, data) => {
      try {
        if (err) {
          ...
          var text = html("div.rrp-box1-left-times").text();
          $.message = text;
          console.log(text);
        }
      } catch (e) {
